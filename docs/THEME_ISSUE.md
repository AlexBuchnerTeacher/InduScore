# Skalierbares Theme-System mit RBS und C64-Theme
...
3. **Web-Fonts**: C64-Font mit Web-Fallback (keine FOIT)
...
│       │   ├── design_tokens.dart          # DesignTokens (colors, typography, spacing)
│       │   ├── theme_bundle.dart           # ThemeBundle (themeData + tokens + microcopy + assets)
│       │   └── microcopy_set.dart          # MicrocopySet (ok, error, saved, ready)
│       ├── tokens/
...
│       └── legacy/
│           └── rbs_theme.dart              # Alte RBS-Definitionen (deprecated)
├── features/
│   └── admin/
│       └── screens/
...
- [ ] `ThemeKey` enum erstellen
- [ ] `DesignTokens` Klasse erstellen
- [ ] `MicrocopySet` Klasse erstellen
- [ ] `ThemeAssets` Klasse erstellen
- [ ] `ThemeBundle` Klasse erstellen
- [ ] `ThemeBundleFactory` Interface erstellen
- [ ] `ThemeFactory` für Material 3 erstellen
...
- [ ] `ThemeRegistry` Singleton erstellen
...
### Phase 5: Firestore & Persistenz (5 Tasks)
